...
| 文件 | 行数 | 职责 |
|------|------|------|
| `agent_types.rs` | 67 | Agent 核心类型定义与 serde 序列化契约 |
| `agent_service.rs` | 491 | Agent CRUD、配置缓存、事务化持久化、同步联动 |
| `avatar_service.rs` | 436 | 头像二进制存储、SHA-256 哈希、主色调提取算法 |
| `agent_chat_application_service.rs` | 227 | Agent 聊天应用层编排：加载配置 → 组装上下文 → 发起流式请求 |
...
`agent_types.rs` 是整个 Agent 领域的**类型契约层**。它仅包含 67 行代码，不做任何业务逻辑，只定义数据结构、序列化行为与默认值策略。这种极简设计保证了上层模块对类型的单向依赖，避免循环引用。
...
| `current_topic_id` | `Option<String>` | `default` (`None`) | 当前活跃话题 ID |
...
- **`agents` 表**：读取 `name`、`system_prompt`、`mobile_system_prompt`、`model`、`temperature`、`context_token_limit`、`max_output_tokens`、`stream_output`、`current_topic_id`
...
| `thinking_message_id` | 预分配的 Assistant 消息 ID，流式响应期间前端以此 ID 占位展示 Thinking 状态 |
...
┌─────────────────────────────────────────────────────────────┐
│ 7. 启动前台服务保活                                          │
│    tauri_plugin_vcp_mobile::stream::start_stream_service_    │
│    inner(&app_handle, &agent_config.name)                     │
│    ──> Android 前台服务，降低 OEM 杀后台概率                   │
└──────────────────────────┬──────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────┐
│ 8. 发起 VCP 流式请求                                         │
│    perform_vcp_request(...)                                  │
│    ──> HTTP SSE 流式解析，通过 Channel 向前端推送 StreamEvent │
└──────────────────────────┬──────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────┐
│ 9. 停止前台服务 + 后处理                                      │
...
- 复用前端预分配的 `thinking_message_id` 作为最终消息的 ID
...
| Thinking Message ID | — | 流式响应开始前预分配的 Assistant 消息 ID，用于前端占位展示 | `agent_chat_application_service` |
