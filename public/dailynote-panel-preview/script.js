(function () {
  const DEFAULT_API_BASE = 'http://127.0.0.1:6005/AdminPanel/dailynote_api';
  const API_BASE_STORAGE_KEY = 'DailyNotePanelPreviewApiBase';
  const MOCK_MODE_STORAGE_KEY = 'DailyNotePanelPreviewMockMode';
  const MOCK_DATA_STORAGE_KEY = 'DailyNotePanelPreviewMockData';
  const PANEL_SW_SCOPE = '/dailynote-panel-preview/';
  const PANEL_SW_URL = `${PANEL_SW_SCOPE}sw.js`;
  const ENABLE_SERVICE_WORKER = new URLSearchParams(window.location.search || '').get('sw') === '1';
  const AUTH_STORAGE_KEY = 'vcp_fallback_auth';
  const AUTH_USERNAME_STORAGE_KEY = 'vcp_fallback_auth_username';
  const MOCK_API_LATENCY_MS = 90;
  const MOVE_CONFIRM_IDLE_LABEL = '确认转移';
  const MOVE_CONFIRM_BUSY_LABEL = '转移中…';
  ...
  function loadApiBase() {
    try {
      ...
      console.warn('[DailyNotePanel] Failed to load API base:', e);
  ...
  function loadMockMode() {
    ...
    if (fromQuery !== null) {
      try {
        localStorage.setItem(MOCK_MODE_STORAGE_KEY, fromQuery ? '1' : '0');
      } catch (e) {
        console.warn('[DailyNotePanel] Failed to persist mock mode:', e);
      }
      return fromQuery;
    }
    ...
    try {
      ...
      console.warn('[DailyNotePanel] Failed to load mock mode:', e);
      return false;
    }
  }

  let mockMode = loadMockMode();
  ...
  const moveTargetTrigger = document.getElementById('move-target-trigger');
  const moveTargetSelectedLabel = document.getElementById('move-target-selected-label');
  ...
  const resetMockDataBtn = document.getElementById('reset-mock-data-btn');
  const mockModeStatus = document.getElementById('mock-mode-status');
  ...
  let pendingMoveTargetCandidates = [];
  ...
  let autoRefreshLoopGeneration = 0;
  ...
  function setApiBaseStatus(message, durationMs) {
  ...
  function setMockModeStatus(message, durationMs) {
    if (!mockModeStatus) return;

    mockModeStatus.textContent = message || '';
    if (message && typeof durationMs === 'number' && durationMs > 0) {
      setTimeout(() => {
        mockModeStatus.textContent = '';
      }, durationMs);
    }
  }
  ...
  function populateApiBaseInput() {
    if (apiBaseInput) {
      apiBaseInput.value = apiBase;
    }
  }

  function applyExternalApiBase(value) {
    if (typeof value !== 'string' || !value.trim()) {
      return false;
    }

    const normalized = persistApiBase(value);
    populateApiBaseInput();
    setApiBaseStatus(`已复用 VCPMobile 全局 VCP 服务器地址：${normalized}`, 3200);
    return true;
  }

  function applyExternalAuthPayload(payload) {
    applyExternalApiBase(payload && payload.apiBase);

    const username = payload && typeof payload.username === 'string'
      ? payload.username.trim()
      : '';
    const token = normalizeStoredBasicToken(payload && payload.token);
    ...
    authRejected = false;
    populateAuthInputs();
    setAuthStatus('已复用 VCPMobile 全局管理员鉴权', 2600);
    hydrateSwAuth().catch(console.warn);

    if (!mockMode) {
      bootstrapProtectedData(true).catch(e => {
        if (!isAuthGuardError(e)) {
          console.error('[DailyNotePanel] external auth bootstrap failed:', e);
        }
      });
    }
  ...
  function handleExternalAuthMessage(event) {
    ...
    if (data.type !== 'VCP_DAILYNOTE_SET_AUTH' || data.source !== 'VCPMobile') return;
    applyExternalAuthPayload(data);
  }
  ...
  async function getPanelServiceWorkerTarget() {
    if (!ENABLE_SERVICE_WORKER || !('serviceWorker' in navigator)) return null;
    ...
    try {
      const registration = await navigator.serviceWorker.getRegistration(PANEL_SW_SCOPE);
      return registration
  ...
  function bindServiceWorkerListeners() {
    if (!ENABLE_SERVICE_WORKER || !('serviceWorker' in navigator) || serviceWorkerListenersBound) return;
  ...
  async function authorizedFetch(path, options) {
    ...
    if (mockMode) {
      throw new Error('Mock mode should not call authorizedFetch');
    }
    ...
    try {
      ...
      throw new Error(`无法连接 DailyNote API：${error && error.message ? error.message : error}`);
    }
  ...
  function startAutoRefreshLoop() {
    ...
    autoRefreshLoopGeneration += 1;
    const generation = autoRefreshLoopGeneration;
    autoRefreshLoop(generation).catch(e => {
      if (generation === autoRefreshLoopGeneration) {
  ...
  async function handleSaveApiBase() {
    ...
    const normalized = persistApiBase(apiBaseInput.value);
    apiBaseInput.value = normalized;
    authRejected = false;
    setApiBaseStatus('API 地址已保存', 2200);

    if (hasConfiguredAuth() || isLegacyRequestMode()) {
      try {
        await bootstrapProtectedData(true);
        setApiBaseStatus('API 地址已保存并完成刷新', 2600);
      } catch (e) {
        if (!isAuthGuardError(e)) {
          console.error('[DailyNotePanel] save API base refresh failed:', e);
          setApiBaseStatus('API 地址已保存，但刷新失败，请检查服务地址或跨域配置', 4200);
        }
      }
    }
  }
  ...
  async function handleResetApiBase() {
    const normalized = clearStoredApiBase();
    populateApiBaseInput();
    authRejected = false;
    setApiBaseStatus(`已恢复默认：${normalized}`, 3000);
    ...
    if (hasConfiguredAuth() || isLegacyRequestMode()) {
      try {
        ...
        if (!isAuthGuardError(e)) {
          console.error('[DailyNotePanel] reset API base refresh failed:', e);
        }
      }
    }
  }

  async function setMockModeEnabled(enabled, options) {
    const opts = options || {};
    mockMode = !!enabled;
    authRejected = false;
    autoRefreshStarted = false;
    ...
    hasBootstrappedData = false;
    try {
      localStorage.setItem(MOCK_MODE_STORAGE_KEY, mockMode ? '1' : '0');
    } catch (e) {
      console.warn('[DailyNotePanel] Failed to save mock mode:', e);
    }
    ...
    if (mockMode) {
      loadMockData();
      setMockModeStatus(
        opts.message || '已启用 Mock 数据：新增、删除、编辑只写入浏览器本地',
        3600
      );
      await bootstrapProtectedData(true);
      return;
    ...
    clearNotebookRuntimeState();
    setMockModeStatus(opts.message || '已关闭 Mock 数据', 2600);

    if (!hasConfiguredAuth() && !isLegacyRequestMode()) {
      enterAuthSafeMode('已关闭 Mock 数据，请配置内化鉴权后连接真实 API');
      return;
    }
  ...
  async function handleResetMockData() {
    resetMockData();
    if (!mockMode) {
      setMockModeStatus('Mock 数据已重置；启用 Mock 后生效', 3200);
      return;
    }
    ...
    notebookLatestMtime.clear();
    await bootstrapProtectedData(true);
    setMockModeStatus('Mock 数据已恢复到初始样本', 2800);
  }
  ...
  function normalizeMockData(rawData) {
    const fallback = { version: 1, folders: {} };
    if (!rawData || typeof rawData !== 'object') return fallback;

    const folders = {};
    Object.entries(rawData.folders || {}).forEach(([folderName, rawNotes]) => {
      if (!folderName || !Array.isArray(rawNotes)) return;
      folders[folderName] = rawNotes
        .filter(note => note && typeof note.name === 'string')
        .map(note => ({
          name: note.name,
          content: String(note.content || ''),
          mtime: Number(note.mtime) || Date.now()
        }));
    });
  ...
  function createMockSeedData() {
    ...
    return normalizeMockData({
      ...
      folders: {
        '日常手记': [
          {
            name: '2026-06-09-08_20_11.txt',
            mtime: minutesAgo(4),
            content:
              '[2026-06-09] - Manual\n\n早上先把 DailyNotePanel 的响应式布局过了一遍。手机宽度下侧边栏默认折叠是对的，卡片操作按钮也没有挤出边界。\n\n- [x] 检查卡片密度\n- [ ] 试一下新增日记\n- [ ] 试一下删除日记'
          },
          {
            name: '2026-06-08-22_15_43.txt',
            mtime: minutesAgo(38),
            content:
              '[2026-06-08] - Manual\n\n今天主要确认 VCPToolBox 的管理员鉴权链路。DailyNotePanel 没有独立账号体系，应该复用 VCPMobile 设置里的管理员账号密码。'
          },
          {
            name: '2026-06-07-19_42_05.txt',
            mtime: minutesAgo(260),
            content:
              '[2026-06-07] - Manual\n\n做了一次桌面与移动视口对照。卡片列表在窄屏下需要更紧凑，设置页的长 URL 要允许换行。'
          }
        ],
        '工作复盘': [
          {
            name: '2026-06-09-10_05_28.txt',
            mtime: minutesAgo(12),
            content:
              '[2026-06-09] - VCPMobile\n\n移植策略先保持克制：静态页验证 UI 与 CRUD 行为，再接右侧抽屉入口。下一步重点是把独立鉴权输入换成设置仓库里的管理员账号密码。'
          },
          {
            name: '2026-06-06-18_31_09.txt',
            mtime: minutesAgo(540),
            content:
              '[2026-06-06] - VCPToolBox\n\n原插件路由只负责挂载静态资源和 dailynote API，真正鉴权由 /AdminPanel 的 Basic Auth 中间件处理。'
          }
        ],
        '灵感簿': [
          {
            name: '2026-06-09-09_36_00.txt',
            mtime: minutesAgo(20),
            content:
              '[2026-06-09] - Idea\n\nMock 数据模式只测试前端常规行为，不触碰真实日记目录。新增、删除、编辑都写入浏览器 localStorage，刷新页面后仍保留。'
          }
        ]
      }
    });
  ...
  function loadMockData() {
    try {
      const raw = localStorage.getItem(MOCK_DATA_STORAGE_KEY);
      if (raw) {
        return normalizeMockData(JSON.parse(raw));
      }
    } catch (e) {
      console.warn('[DailyNotePanel] Failed to load mock data:', e);
    }

    const seeded = createMockSeedData();
    persistMockData(seeded);
    return seeded;
  }

  function persistMockData(data) {
    try {
      localStorage.setItem(
        MOCK_DATA_STORAGE_KEY,
        JSON.stringify(normalizeMockData(data))
      );
    } catch (e) {
      console.warn('[DailyNotePanel] Failed to persist mock data:', e);
    }
  }
  ...
  function resetMockData() {
    try {
      ...
      console.warn('[DailyNotePanel] Failed to reset mock data:', e);
    }
    return loadMockData();
  }

  function getMockPathSegments(path) {
  ...
  function buildMockPreview(content) {
  ...
  function toMockListNote(folderName, note) {
    const content = String(note && note.content ? note.content : '');
    return {
      folderName,
      name: note.name,
      mtime: Number(note.mtime) || 0,
      size: new TextEncoder().encode(content).length,
      preview: buildMockPreview(content)
    };
  }

  function findMockNote(data, folderName, fileName) {
    const list = data.folders[folderName] || [];
    return list.find(note => note && note.name === fileName) || null;
  }

  async function withMockLatency(result) {
  ...
  async function mockApiGet(path) {
    ...
    const url = new URL(String(path || '/'), window.location.origin);
    const segments = getMockPathSegments(path);

    if (segments[0] === 'folders') {
      return withMockLatency({ folders: Object.keys(data.folders) });
    }

    if (segments[0] === 'folder' && segments[1]) {
      const folderName = segments[1];
      const notesInFolder = data.folders[folderName] || [];
      return withMockLatency({
        notes: notesInFolder.map(note => toMockListNote(folderName, note))
      });
    }

    if (segments[0] === 'note' && segments[1] && segments[2]) {
      const folderName = segments[1];
      const fileName = segments.slice(2).join('/');
      const note = findMockNote(data, folderName, fileName);
      if (!note) {
        throw new Error('404 Mock note not found');
      }
      return withMockLatency({ content: note.content || '' });
    }
    ...
    if (segments[0] === 'search') {
      ...
      Object.entries(data.folders).forEach(([folderName, notesInFolder]) => {
        if (folderFilter && folderName !== folderFilter) return;
        notesInFolder.forEach(note => {
          const haystack = `${note.name}\n${note.content || ''}`.toLowerCase();
          if (keywords.every(keyword => haystack.includes(keyword))) {
            matches.push(toMockListNote(folderName, note));
          }
        });
      });

      matches.sort((a, b) => b.mtime - a.mtime);
      return withMockLatency({ notes: matches });
    }

    throw new Error(`404 Unsupported mock GET: ${path}`);
  }
  ...
  async function mockApiPost(path, body) {
    ...
    if (segments[0] === 'note' && segments[1] && segments[2]) {
      ...
      const list = data.folders[folderName] || [];
      const existing = list.find(note => note.name === fileName);
      ...
      data.folders[folderName] = list;
      persistMockData(data);
      return withMockLatency({ success: true });
    }

    if (segments[0] === 'delete-batch') {
      const deleted = [];
      const errors = [];
      const notesToDelete = Array.isArray(body && body.notesToDelete)
        ? body.notesToDelete
        : [];

      notesToDelete.forEach(item => {
        const folderName = item && item.folder;
        const fileName = item && item.file;
        const noteId = getNoteId(folderName, fileName);
        const list = data.folders[folderName] || [];
        const beforeLength = list.length;
        data.folders[folderName] = list.filter(note => note.name !== fileName);

        if (data.folders[folderName].length < beforeLength) {
          deleted.push(noteId);
        } else {
          errors.push({ note: noteId, error: 'not found' });
        }
      });

      persistMockData(data);
      return withMockLatency({ deleted, errors });
    }

    if (segments[0] === 'move') {
      const moved = [];
      const errors = [];
      const sourceNotes = Array.isArray(body && body.sourceNotes)
        ? body.sourceNotes
        : [];
      const targetFolder = String((body && body.targetFolder) || '').trim();

      if (!targetFolder) {
        throw new Error('400 Missing mock move target folder');
      }

      data.folders[targetFolder] = data.folders[targetFolder] || [];

      sourceNotes.forEach(item => {
        const folderName = item && item.folder;
        const fileName = item && item.file;
        const noteId = getNoteId(folderName, fileName);
        const sourceList = data.folders[folderName] || [];
        const sourceIndex = sourceList.findIndex(note => note.name === fileName);
        ...
        const [note] = sourceList.splice(sourceIndex, 1);
        note.mtime = Date.now();

        const targetList = data.folders[targetFolder];
        const targetIndex = targetList.findIndex(item => item.name === fileName);
        if (targetIndex >= 0) {
          targetList[targetIndex] = note;
        } else {
      ...
      persistMockData(data);
      return withMockLatency({ moved, errors });
    }

    throw new Error(`404 Unsupported mock POST: ${path}`);
  }
  ...
  function getSuccessfulMutationFiles(files, result, successKey) {
    const erroredIds = getErroredNoteIdSet(result && result.errors);
    const successIds = Array.isArray(result && result[successKey])
      ? new Set(result[successKey].map(item => String(item)))
      : null;

    return (files || []).filter(item => {
      if (!item || !item.folder || !item.file) return false;
      const noteId = getNoteId(item.folder, item.file);
      if (erroredIds.has(noteId)) return false;
      return !successIds || successIds.has(noteId);
    });
  }

  function setMoveSubmitting(isSubmitting) {
    if (!moveConfirmBtn) return;
    moveConfirmBtn.disabled = isSubmitting || !pendingMoveTargetFolder;
    moveConfirmBtn.textContent = isSubmitting
      ? MOVE_CONFIRM_BUSY_LABEL
      : MOVE_CONFIRM_IDLE_LABEL;
  }
  ...
  function closeMoveTargetOptions() {
    if (moveTargetOptions) {
      moveTargetOptions.classList.add('hidden');
    }
    if (moveTargetTrigger) {
      moveTargetTrigger.setAttribute('aria-expanded', 'false');
    }
  }

  function openMoveTargetOptions() {
    if (
      !moveTargetOptions ||
      !moveTargetTrigger ||
      moveTargetTrigger.disabled ||
      pendingMoveTargetCandidates.length === 0
    ) {
      return;
    }

    moveTargetOptions.classList.remove('hidden');
    moveTargetTrigger.setAttribute('aria-expanded', 'true');
  }

  function toggleMoveTargetOptions() {
    if (!moveTargetOptions || moveTargetOptions.classList.contains('hidden')) {
      openMoveTargetOptions();
      return;
    }

    closeMoveTargetOptions();
  }

  function selectMoveTargetFolder(folderName) {
    pendingMoveTargetFolder = folderName || null;

    if (moveTargetSelectedLabel) {
      moveTargetSelectedLabel.textContent = pendingMoveTargetFolder || '请选择目标日记本';
    }

    setMoveSubmitting(false);

    if (moveTargetOptions) {
      Array.from(moveTargetOptions.querySelectorAll('.modal-option-item')).forEach(button => {
        const selected = button.dataset.value === pendingMoveTargetFolder;
  ...
  function renderMoveTargetOptions(targetCandidates) {
    pendingMoveTargetCandidates = Array.isArray(targetCandidates)
      ? targetCandidates.slice()
      : [];

    if (moveTargetSelectedLabel) {
      moveTargetSelectedLabel.textContent = pendingMoveTargetCandidates.length > 0
        ? '请选择目标日记本'
        : '无可用目标日记本';
    }

    if (moveTargetTrigger) {
      moveTargetTrigger.disabled = pendingMoveTargetCandidates.length === 0;
      moveTargetTrigger.setAttribute('aria-expanded', 'false');
    }

    if (!moveTargetOptions) return;

    moveTargetOptions.innerHTML = '';
    moveTargetOptions.classList.add('hidden');

    pendingMoveTargetCandidates.forEach(nb => {
      const button = document.createElement('button');
      button.type = 'button';
      button.className = 'modal-option-item';
      button.dataset.value = nb.name;
      button.textContent = nb.name;
      button.setAttribute('role', 'option');
      button.setAttribute('aria-selected', 'false');
      button.addEventListener('click', () => {
        selectMoveTargetFolder(nb.name);
      });
      moveTargetOptions.appendChild(button);
    });
  }
  ...
  function getAuthRequirementHint() {
    return mockMode
      ? 'Mock 数据模式已启用，当前操作只影响浏览器本地样本'
      : '请先在设置页顶部配置可用的内化鉴权';
  }
  ...
  function renderCards() {
    ...
    slice.forEach(note => {
      ...
      if (isStreamNotebook(currentNotebook)) {
        ...
        const maidTitle = document.createElement('span');
        maidTitle.className = 'stream-card-title-maid';
        maidTitle.textContent = ` - ${maidName}`;
        title.append(folderTitle, maidTitle);
  ...
  function bindEvents() {
    ...
    if (saveApiBaseBtn) {
      saveApiBaseBtn.addEventListener('click', () => {
        handleSaveApiBase().catch(console.error);
      });
    }

    if (resetApiBaseBtn) {
      resetApiBaseBtn.addEventListener('click', () => {
        handleResetApiBase().catch(console.error);
      });
    }
    ...
    if (mockModeCheckbox) {
      mockModeCheckbox.addEventListener('change', () => {
        setMockModeEnabled(mockModeCheckbox.checked).catch(e => {
          console.error('[DailyNotePanel] toggle mock mode failed:', e);
          setMockModeStatus('切换 Mock 数据失败，请稍后重试', 3200);
        });
      });
    }

    if (resetMockDataBtn) {
      resetMockDataBtn.addEventListener('click', () => {
        handleResetMockData().catch(e => {
          console.error('[DailyNotePanel] reset mock data failed:', e);
          setMockModeStatus('重置 Mock 数据失败，请稍后重试', 3200);
        });
      });
    }
    ...
    if (moveTargetTrigger) {
      moveTargetTrigger.addEventListener('click', () => {
        toggleMoveTargetOptions();
      });

      moveTargetTrigger.addEventListener('keydown', event => {
    ...
    if (moveTargetOptions) {
      moveTargetOptions.addEventListener('keydown', event => {
        if (event.key === 'Escape') {
          event.preventDefault();
          closeMoveTargetOptions();
          if (moveTargetTrigger) moveTargetTrigger.focus();
        }
      });
    }

    if (moveModalBackdrop) {
      moveModalBackdrop.addEventListener('click', event => {
        if (
          (moveTargetTrigger && moveTargetTrigger.contains(event.target)) ||
          (moveTargetOptions && moveTargetOptions.contains(event.target))
        ) {
          return;
        }
        closeMoveTargetOptions();
    ...
    if (moveConfirmBtn) {
      moveConfirmBtn.addEventListener('click', async () => {
        ...
        const errorCount = Array.isArray(result.errors) ? result.errors.length : 0;
        const erroredIds = getErroredNoteIdSet(result.errors);
        const movedFiles = getSuccessfulMutationFiles(filesToMove, result, 'moved');
        const currentWorkbenchNoteId = workbenchState.noteId;
        const affectsCurrentWorkbench =
          workbenchMode &&
          !!currentWorkbenchNoteId &&
          doesFileCollectionContainNote(movedFiles, currentWorkbenchNoteId) &&
          !erroredIds.has(currentWorkbenchNoteId);
        const affectedFolders = new Set(
          movedFiles.map(item => item.folder).filter(Boolean)
        );
        ...
        applyLocalMovePatch(movedFiles, targetFolder);
        updateSidebarGlow();
        await rebuildCurrentViewAfterMutation();
        if (affectsCurrentWorkbench) {
          clearWorkbenchState({ renderCards: false });
  ...
  function updateSearchUIForCurrentNotebook() {
    ...
    if (disabledByAuth) {
      ...
      if (createNoteButton) {
        ...
        createNoteButton.title = getAuthRequirementHint();
      }
      return;
    }
  ...
  function openMoveModal(files) {
    ...
    renderMoveTargetOptions(targetCandidates);
  ...
  function closeMoveModal() {
    pendingMoveFiles = [];
    pendingMoveTargetFolder = null;
    pendingMoveTargetCandidates = [];
    if (moveTargetTrigger) {
      moveTargetTrigger.disabled = false;
      moveTargetTrigger.setAttribute('aria-expanded', 'false');
    }
    if (moveTargetSelectedLabel) {
      moveTargetSelectedLabel.textContent = '请选择目标日记本';
    }
    if (moveTargetOptions) {
      moveTargetOptions.innerHTML = '';
      moveTargetOptions.classList.add('hidden');
    ...
    if (moveConfirmBtn) {
      ...
      moveConfirmBtn.textContent = MOVE_CONFIRM_IDLE_LABEL;
    }
    if (!moveModalBackdrop) return;
  ...
  async function autoRefreshLoop(generation) {
    const INTERVAL = 10000; // 10 秒
    while (true) {
      try {
        if (!autoRefreshStarted || generation !== autoRefreshLoopGeneration) {
          return;
        }
  ...
  async function init() {
    ...
    if (mockMode) {
      loadMockData();
      setMockModeStatus('Mock 数据已启用：可直接测试新增、编辑和删除', 3200);
    }
