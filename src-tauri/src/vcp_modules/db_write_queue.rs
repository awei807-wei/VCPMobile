...
pub enum DbWriteTask {
    UpsertAgent { id: String, dto: AgentSyncDTO },
    UpsertGroup { id: String, dto: GroupSyncDTO },
    UpsertAvatar { owner_type: String, owner_id: String, bytes: Vec<u8> },
    UpsertAgentTopic { topic_id: String, dto: AgentTopicSyncDTO },
    UpsertGroupTopic { topic_id: String, dto: GroupTopicSyncDTO },
    UpsertMessages { topic_id: String, owner_id: String, owner_type: String, messages: Vec<ChatMessage> },
}
...
impl DbWriteQueue {
        ...
        tokio::spawn(async move {
            ...
            while let Some(task) = rx.recv().await {
                match task {
                    ...
                    DbWriteTask::UpsertAvatar { owner_type, owner_id, bytes } => {
                        if let Err(e) = Self::upsert_avatar(&pool, &owner_type, &owner_id, &bytes).await {
                            println!("[DbWriteQueue] Avatar {}:{} error: {}", owner_type, owner_id, e);
                        }
                    }
                    ...
                    DbWriteTask::UpsertMessages { topic_id, owner_id, owner_type, messages } => {
                        if let Err(e) = Self::upsert_messages(&pool, &topic_id, &owner_id, &owner_type, &messages).await {
    ...
    async fn upsert_agent(pool: &sqlx::SqlitePool, id: &str, dto: &AgentSyncDTO) -> Result<(), String> {
        ...
        println!("[DbWriteQueue] Upserted Agent: {}", id);
        Ok(())
    }

    async fn upsert_group(pool: &sqlx::SqlitePool, id: &str, dto: &GroupSyncDTO) -> Result<(), String> {
        let now = chrono::Utc::now().timestamp_millis();
        ...
        tx.commit().await.map_err(|e| e.to_string())?;
        println!("[DbWriteQueue] Upserted Group: {}", id);
        Ok(())
    }

    async fn upsert_avatar(pool: &sqlx::SqlitePool, owner_type: &str, owner_id: &str, bytes: &[u8]) -> Result<(), String> {
        ...
        println!("[DbWriteQueue] Upserted Avatar: {}:{}", owner_type, owner_id);
        Ok(())
    }

    async fn upsert_agent_topic(pool: &sqlx::SqlitePool, topic_id: &str, dto: &AgentTopicSyncDTO) -> Result<(), String> {
        ...
        if !owner_exists {
            println!("[DbWriteQueue] Skip agent_topic {}: owner {} not found", topic_id, dto.owner_id);
            return Ok(());
        }
        ...
        println!("[DbWriteQueue] Upserted Agent Topic: {}", topic_id);
        Ok(())
    }

    async fn upsert_group_topic(pool: &sqlx::SqlitePool, topic_id: &str, dto: &GroupTopicSyncDTO) -> Result<(), String> {
        let owner_exists: bool = sqlx::query_scalar(
        ...
        if !owner_exists {
            println!("[DbWriteQueue] Skip group_topic {}: owner {} not found", topic_id, dto.owner_id);
            return Ok(());
        }
        ...
        println!("[DbWriteQueue] Upserted Group Topic: {}", topic_id);
        Ok(())
    }
    ...
    async fn upsert_messages(
        ...
        for msg in messages {
            let _ = message_service::patch_single_message_no_app(pool, owner_id, owner_type, topic_id.to_string(), msg.clone(), false).await;
        }
        ...
        println!("[DbWriteQueue] Upserted {} messages for Topic: {}", messages.len(), topic_id);
        Ok(())
    }
}
