...
#[tokio::test]
async fn group_topic_upsert_and_delete_are_owner_scoped() {
    ...
    sqlx::query(
        "INSERT INTO topics
         ...
         VALUES
            ('agent', 'shared-owner', 'shared-topic', 'agent', 1, 1, 1, 0),
            ('group', 'shared-owner', 'shared-topic', 'old', 1, 1, 1, 0)",
    )
    .execute(&pool)
    .await
    .unwrap();
    ...
    let agent_topic = sqlx::query(
        "SELECT title, deleted_at FROM topics
         WHERE owner_type = 'agent' AND owner_id = 'shared-owner' AND topic_id = 'shared-topic'",
    )
    .fetch_one(&pool)
    .await
    .unwrap();
    assert_eq!(agent_topic.get::<String, _>("title"), "agent");
    assert!(agent_topic.get::<Option<i64>, _>("deleted_at").is_none());
    let agent_message = sqlx::query(
    ...
    .await
    .unwrap();
    assert!(agent_message.get::<Option<i64>, _>("deleted_at").is_none());
    let agent_generation_count: i64 = sqlx::query_scalar(
    ...
    .await
    .unwrap();
    assert_eq!(agent_generation_count, 1);
}
