...
async fn ensure_attachments_locally(app: &AppHandle, message: &mut ChatMessage) -> Result<(), String> {
    ...
    if !att_dir.exists() { fs::create_dir_all(&att_dir).await.map_err(|e| e.to_string())?; }
...
pub async fn delete_messages(
    ...
    if msg_ids.is_empty() { return Ok(()); }
    let mut tx = db_pool.begin().await.map_err(|e| e.to_string())?;
    let delete_query = format!("UPDATE messages SET deleted_at = ? WHERE topic_id = ? AND msg_id IN ({})",
        msg_ids.iter().map(|_| "?").collect::<Vec<_>>().join(", "));
    let now = chrono::Utc::now().timestamp_millis();
    let mut q = sqlx::query(&delete_query).bind(now).bind(topic_id);
    for id in &msg_ids { q = q.bind(id); }
