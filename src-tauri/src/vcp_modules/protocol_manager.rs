...
pub fn register_vcp_protocols<R: Runtime>(builder: tauri::Builder<R>) -> tauri::Builder<R> {
    builder
        ...
        .register_uri_scheme_protocol("vcp", |ctx, request| handle_vcp_request(ctx, request))
...
pub async fn prepare_vcp_upload<R: Runtime>(
    ...
    log::info!(
        "[ProtocolManager] High-speed link listening on port {}",
        port
    );
    tauri::async_runtime::spawn(async move {
        log::info!(
            "[ProtocolManager] Spawned upload task for {}",
            metadata.name
        );
        ...
        while !upload_finished && start_time.elapsed() < timeout {
            ...
            loop {
                ...
                let data = if !body_started {
                    ...
                    if let Some(pos) = header_data.windows(4).position(|w| w == b"\r\n\r\n") {
                        ...
                        // POST 请求：在此处创建文件
                        log::info!(
                            "[ProtocolManager] Data stream started, creating temp file: {:?}",
                            temp_file_path
                        );
                        ...
                        let header_len_in_current_buffer = if header_data.len() > n {
                            ...
                            if pos + 4 > consumed_before {
                                pos + 4 - consumed_before
                            } else {
                                0
                            }
                        } else {
                ...
                if !data.is_empty() && !is_options {
                    ...
                    if bytes_count >= metadata.size {
                        log::info!(
                            "[ProtocolManager] Stream complete: {}/{}",
                            bytes_count,
                            metadata.size
                        );
                        break;
            ...
            if !is_options && bytes_count > 0 {
                ...
                if bytes_count < metadata.size {
                    log::warn!(
                        "[ProtocolManager] Upload interrupted or incomplete: {}/{} bytes",
                        bytes_count,
                        metadata.size
                    );
                    ...
                    log::info!(
                        "[ProtocolManager] Finalizing upload, total: {} bytes",
                        bytes_count
                    );
