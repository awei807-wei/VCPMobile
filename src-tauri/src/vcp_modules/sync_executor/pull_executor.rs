...
impl PullExecutor {
    pub async fn pull_agent<R: Runtime>(
        ...
        write_queue.submit(DbWriteTask::UpsertAgent { id: agent_id.to_string(), dto }).await;
        
        println!("[PullExecutor] Queued Agent: {}", agent_id);
        Ok(())
    }
    ...
    pub async fn pull_group<R: Runtime>(
        ...
        let url = format!("{}/api/mobile-sync/download-entity?id={}&type=group", http_url, group_id);
        let res = client
            .get(&url)
            .header("x-sync-token", sync_token)
            .send()
        ...
        write_queue.submit(DbWriteTask::UpsertGroup { id: group_id.to_string(), dto }).await;
        
        println!("[PullExecutor] Queued Group: {}", group_id);
        Ok(())
    }
    ...
    pub async fn pull_avatar<R: Runtime>(
        ...
        write_queue.submit(DbWriteTask::UpsertAvatar { 
            owner_type: owner_type.to_string(), 
            owner_id: owner_id.to_string(), 
            bytes: bytes.to_vec() 
        }).await;

        println!("[PullExecutor] Queued Avatar: {}:{}", owner_type, owner_id);
        Ok(())
    }
    ...
    pub async fn pull_agent_topic<R: Runtime>(
        ...
        let url = format!("{}/api/mobile-sync/download-entity?id={}&type=agent_topic", http_url, topic_id);
        let res = client
            .get(&url)
            .header("x-sync-token", sync_token)
            .send()
        ...
        write_queue.submit(DbWriteTask::UpsertAgentTopic { topic_id: topic_id.to_string(), dto }).await;

        println!("[PullExecutor] Queued Agent Topic: {}", topic_id);
        Ok(())
    }
    ...
    pub async fn pull_group_topic<R: Runtime>(
        ...
        let url = format!("{}/api/mobile-sync/download-entity?id={}&type=group_topic", http_url, topic_id);
        let res = client
            .get(&url)
            .header("x-sync-token", sync_token)
            .send()
        ...
        write_queue.submit(DbWriteTask::UpsertGroupTopic { topic_id: topic_id.to_string(), dto }).await;

        println!("[PullExecutor] Queued Group Topic: {}", topic_id);
        Ok(())
    }
    ...
    pub async fn pull_messages(
                            ...
                            if let Some(hash) = att_obj.get("hash").and_then(|h| h.as_str()) {
                                if !hash.is_empty() {
                                    ...
                                    if let Some(path) = existing_path {
                                        att_obj.entry("internalPath".to_string()).or_insert(serde_json::json!(path));
                                        att_obj.entry("src".to_string()).or_insert(serde_json::json!(format!("file://{}", path)));
                                    } else {
                                        let default_path = format!("file://attachments/{}", hash);
                                        att_obj.entry("internalPath".to_string()).or_insert(serde_json::json!(default_path.trim_start_matches("file://")));
                                        att_obj.entry("src".to_string()).or_insert(serde_json::json!(default_path));
                                    }
                                }
                            }
                            att_obj.entry("status".to_string()).or_insert(serde_json::json!("ready"));
                        }
                    }
                }
        ...
        write_queue.submit(DbWriteTask::UpsertMessages { 
        ...
        println!("[PullExecutor] Queued {} messages for Topic: {}", msg_ids.len(), topic_id);
        Ok(())
    }
}
