  ...
  <div class="vcp-attachment-preview flex flex-col gap-2 mt-2 w-full max-w-full overflow-hidden">
    <div v-for="(att, index) in attachments" :key="index" class="attachment-item">
      ...
      <div v-if="att.type.startsWith('image/')" class="image-preview relative group" @click="openViewer(att)">
        <img :src="getAttachmentRenderSrc(att)" :alt="att.name"
      ...
      <!-- 音频类型 -->
      <div v-else-if="att.type.startsWith('audio/')"
      ...
      <!-- 视频类型 -->
      <div v-else-if="att.type.startsWith('video/')"
      ...
      <div v-else
        ...
        <div class="flex flex-col overflow-hidden flex-1">
          ...
          <div class="flex items-center gap-2 text-[10px] opacity-40">
            ...
            <span v-if="att.extractedText"
              class="px-1.5 py-0.5 bg-blue-500/20 text-blue-400 rounded-sm scale-90">TEXT_EXTRACTED</span>
          </div>
        </div>
    ...
    <!-- 内置轻量预览器 -->
    <AttachmentViewer :file="activeFile" :is-open="isViewerOpen" @close="isViewerOpen = false"
