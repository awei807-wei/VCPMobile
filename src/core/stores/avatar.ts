    ...
    img.onload = () => {
      try {
        ...
        for (let i = 0; i < imgData.length; i += 4) {
          ...
          const bucket = colorBuckets.get(binKey) || {
            r: 0,
            g: 0,
            b: 0,
            count: 0,
        ...
        console.error(
          "[AvatarStore] Canvas dominant color computation error:",
          e
        );
        resolve("#808080");
      }
...
export const useAvatarStore = defineStore("avatar", () => {
            ...
            if (!inFlightCompute.has(key)) {
              ...
              extractDominantColorFromBlob(tempBlobUrl)
                .then((color) => {
                  ...
                  return invoke("store_dominant_color", {
                    ownerType,
                    ownerId,
                    color,
                  });
                ...
                .then(() => {
                  console.log(
                    `[AvatarStore] Computed and stored dominant_color for ${key}`
                  );
                })
                .catch((err) => {
                  console.error(
                    `[AvatarStore] Failed to handle dominant_color for ${key}:`,
                    err
                  );
                })
                .finally(() => {
  ...
  const preloadAll = async (): Promise<void> => {
    ...
    try {
      ...
      console.log(
        `[AvatarStore] preloadAll complete in ${
          Date.now() - startTime
        }ms. Cached ${results?.length || 0} avatars.`
      );
    } catch (err) {
