...
export const useChatSessionStore = defineStore(
  ...
  () => {
    ...
    const readLastActiveTopic = (
      ownerType: ConversationOwnerType,
      ownerId: string,
    ): string | undefined => {
      const compositeKey = ownerMapKey(ownerType, ownerId);
      const current = lastActiveTopicMap.value[compositeKey];
      if (current) return current;
      const legacy = lastActiveTopicMap.value[ownerId];
      if (!legacy) return undefined;
      lastActiveTopicMap.value[compositeKey] = legacy;
      delete lastActiveTopicMap.value[ownerId];
      return legacy;
    };

    const rememberLastActiveTopic = (
      ownerType: ConversationOwnerType,
      ownerId: string,
      topicId: string,
    ): void => {
      lastActiveTopicMap.value[ownerMapKey(ownerType, ownerId)] = topicId;
      delete lastActiveTopicMap.value[ownerId];
    };
    ...
    const selectTopicById = async (
      ...
      rememberLastActiveTopic(ownerType, itemId, topicId);
    ...
    const selectItem = async (
      ...
      let targetTopicId = readLastActiveTopic(ownerType, ownerId);
