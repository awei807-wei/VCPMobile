...
function renderInline(node: InlineNode): string {
  switch (node.type) {
    ...
    case "vcp_custom": {
      const renderedChildren = (node.children || []).map(renderInline).join("");
      const renderedValue = escapeHtml(node.value || "");
      switch (node.kind) {
        case "quote":
          return `<span class="highlighted-quote">${renderedChildren || renderedValue}</span>`;
        case "highlight":
          return `<span class="highlighted-tag">${node.value !== undefined ? renderedValue : renderedChildren}</span>`;
        case "alert":
          return `<span class="highlighted-alert-tag">${node.value !== undefined ? renderedValue : renderedChildren}</span>`;
        default:
          return renderedChildren || renderedValue;
      }
    }
