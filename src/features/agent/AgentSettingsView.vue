      ...
      <section class="space-y-3">
        ...
        <div v-if="sections.params" class="card-modern space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <div class="grid grid-cols-2 gap-5">
            <div>
              <label class="text-[10px] uppercase font-bold opacity-40 mb-2 block">Top P ({{ agentConfig.top_p
                }})</label>
              <input type="range" v-model.number="agentConfig.top_p" min="0" max="1" step="0.05"
                class="w-full accent-blue-500" />
            </div>
            <div>
              <label class="text-[10px] uppercase font-bold opacity-40 mb-2 block">Top K ({{ agentConfig.top_k
                }})</label>
              <input type="number" v-model.number="agentConfig.top_k" min="0" max="64"
        ...
        <div v-if="sections.tts" class="card-modern space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
          <div class="space-y-4">
            <div>
              ...
              <select v-model="agentConfig.ttsVoicePrimary"
            ...
            <div class="border-t border-white/5 pt-4">
              ...
              <select v-model="agentConfig.ttsVoiceSecondary"
                class="w-full bg-black/5 dark:bg-white/5 rounded-xl px-4 py-3 text-sm outline-none appearance-none">
            ...
            <div class="border-t border-white/5 pt-4">
              <label class="text-[10px] uppercase font-bold opacity-40 mb-2 block">语速 ({{
                agentConfig.ttsSpeed.toFixed(1) }}x)</label>
              <input type="range" v-model.number="agentConfig.ttsSpeed" min="0.5" max="2.0" step="0.1"
