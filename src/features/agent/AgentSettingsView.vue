...
import AgentModelParameters from "./AgentModelParameters.vue";
...
<template>
  <SlidePage :is-open="props.isOpen" :z-index="props.zIndex">
    <div
      class="agent-settings-view flex flex-col h-full w-full bg-secondary-bg text-primary-text pointer-events-auto"
    >
      <!-- Header -->
      <header
      ...
      <div class="flex-1 overflow-y-auto p-5 space-y-6 pb-safe no-rubber-band">
        ...
        <section class="space-y-3">
          ...
          <div class="card-modern">
            <textarea
              v-model="agentConfig.mobileSystemPrompt"
              placeholder="在这里输入移动端专用提示词..."
            ...
            <p class="mt-3 text-[10px] opacity-30 px-1 leading-normal">
              提示：此处编辑的提示词仅在本机生效，不会同步到桌面端。留空则使用桌面端同步的提示词。支持
              <code v-pre>{{ AgentName }}</code> 占位符。
            </p>
          </div>
        </section>

        <!-- 3. Model Parameters (Collapsible) -->
        <AgentModelParameters :config="agentConfig" />
...
<style scoped src="./AgentSettingsView.css"></style>
