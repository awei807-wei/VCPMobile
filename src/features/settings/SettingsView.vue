    ...
    <Transition name="fade">
      <div v-if="props.isOpen"
        ...
        <div v-else class="flex-1 overflow-y-auto p-5 space-y-6 pb-safe">
          ...
          <!-- 核心连接 -->
          <SettingsDisclosure title="核心连接" description="VCP Server API 与 WebSocket 鉴权" :default-open="true"
            accent-color="bg-blue-500">
            <VcpCoreSettingsSection :settings="settings" @save-request="saveSettings" />
          </SettingsDisclosure>

          <!-- 数据同步 -->
          <SettingsDisclosure title="数据同步" description="连接桌面端同步插件" accent-color="bg-green-500">
            <SyncSettingsSection :settings="settings" @save-request="saveSettings" @open-sync="openSyncView" />
          </SettingsDisclosure>

          <!-- 话题总结 -->
          <SettingsDisclosure title="话题总结" description="配置总结专用模型" accent-color="bg-yellow-500">
          ...
          <SettingsActionButton variant="primary" size="lg" full-width @click="saveSettings">
            保存并应用变更
        ...
        <ModelSelector :model-value="showSummaryModelSelector" @update:model-value="showSummaryModelSelector = $event"
          :current-model="settings.topicSummaryModel" title="选择总结专用模型" @select="onSummaryModelSelect" />
      </div>
    </Transition>
  </Teleport>
</template>
