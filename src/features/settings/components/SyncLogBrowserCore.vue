...
import { shareFileNative, writeTempFile } from "tauri-plugin-vcp-mobile";
import {
  FileText,
  Trash2,
  Copy,
...
import { sanitizeDiagnosticText } from "../../../core/stores/syncSession/diagnostics";
...
const openFile = async (filename: string) => {
  ...
  try {
    ...
    totalPages.value = Math.max(
      1,
      Math.ceil(lines.value.length / linesPerPage),
    );
    currentPage.value = 0;
...
const copyCurrentFile = async () => {
  ...
  try {
    await navigator.clipboard.writeText(
      sanitizeDiagnosticText(fileContent.value),
    );
    notificationStore.addNotification({
      type: "success",
      message: "同步日志已复制",
...
const shareCurrentFile = async () => {
  ...
  try {
    const safeLog = sanitizeDiagnosticText(fileContent.value);
    const path = await writeTempFile(
      new TextEncoder().encode(safeLog),
      `vcp-sync-log-${Date.now()}.txt`,
    );
...
const clearOldLogs = async () => {
  ...
  try {
    const result = await invoke<LogCleanupResult>("clear_old_sync_logs", {
      keepDays: 7,
    });
    await loadFiles();
    notificationStore.addNotification({
...
const lineClass = (line: string) => {
  ...
  if (line.includes("[TRACE]") || line.includes("[DEBUG]"))
    return "text-white/40";
  if (line.includes("[INFO]") && /success|completed/i.test(line))
...
<template>
  <div class="h-full flex flex-col overflow-hidden">
    ...
    <div v-if="!currentFile" class="flex-1 overflow-y-auto no-rubber-band">
      <div
        v-if="loading"
        class="flex items-center justify-center h-32 text-white/30 text-xs"
      >
        加载中...
      </div>
      <div
        v-else-if="errorText"
        class="mx-4 mt-4 flex items-center justify-between gap-3 border-l-2 border-red-500 bg-red-500/6 px-3 py-2 text-[11px] text-red-300"
      >
        <span>{{ errorText }}</span>
        <button
      ...
      <div
        ...
        class="flex flex-col items-center justify-center h-64 text-white/20 text-xs"
      >
    ...
    <div v-else class="flex-1 flex flex-col overflow-hidden">
      ...
      >
        <button
          @click="closeFile"
          class="flex min-h-9 items-center gap-1 text-[10px] text-white/50 hover:text-white"
        >
          <ChevronLeft :size="14" />
          返回列表
        </button>
      ...
      <div
        class="flex-1 overflow-y-auto overflow-x-auto px-4 py-3 font-mono text-[10px] leading-relaxed min-w-0 no-rubber-band"
      >
        <div
      ...
      <div
        v-if="totalPages > 1"
        class="flex items-center justify-between px-4 py-2 border-t border-white/10 text-[10px]"
      >
        <button
          @click="prevPage"
